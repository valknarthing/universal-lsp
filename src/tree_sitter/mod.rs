...
static LANGUAGE_REGISTRY: Lazy<DashMap<String, Language>> = Lazy::new(|| {
    ...
    // JVM languages
    registry.insert("scala".to_string(), tree_sitter_scala::language());
    registry.insert("kotlin".to_string(), tree_sitter_kotlin::language());

    // .NET languages
    registry.insert("csharp".to_string(), tree_sitter_c_sharp::language());
    registry.insert("cs".to_string(), tree_sitter_c_sharp::language());  // Alias

    // Apple ecosystem
    registry.insert("swift".to_string(), tree_sitter_swift::language());

    // Functional programming
    registry.insert("elixir".to_string(), tree_sitter_elixir::language());
    registry.insert("ex".to_string(), tree_sitter_elixir::language());  // Alias
    registry.insert("ocaml".to_string(), tree_sitter_ocaml::language_ocaml());
    registry.insert("erlang".to_string(), tree_sitter_erlang::language());
    registry.insert("erl".to_string(), tree_sitter_erlang::language());  // Alias

    // Scientific computing
    registry.insert("julia".to_string(), tree_sitter_julia::language());
    registry.insert("jl".to_string(), tree_sitter_julia::language());  // Alias

    // Scripting & embeddable
    registry.insert("lua".to_string(), tree_sitter_lua::language());

    // DevOps
    registry.insert("dockerfile".to_string(), tree_sitter_dockerfile::language());
...
impl TreeSitterParser {
    ...
    pub fn extract_symbols(&self, tree: &Tree, source: &str, lang: &str) -> Result<Vec<Symbol>> {
        ...
        match lang {
            ...
            "scala" => {
                self.extract_scala_symbols(root_node, source, &mut symbols)?;
            }
            "kotlin" => {
                self.extract_kotlin_symbols(root_node, source, &mut symbols)?;
            }
            "csharp" | "cs" => {
                self.extract_csharp_symbols(root_node, source, &mut symbols)?;
            }
            "swift" => {
                self.extract_swift_symbols(root_node, source, &mut symbols)?;
            }
            "elixir" | "ex" => {
                self.extract_elixir_symbols(root_node, source, &mut symbols)?;
            }
            "ocaml" => {
                self.extract_ocaml_symbols(root_node, source, &mut symbols)?;
            }
            "erlang" | "erl" => {
                self.extract_erlang_symbols(root_node, source, &mut symbols)?;
            }
            "julia" | "jl" => {
                self.extract_julia_symbols(root_node, source, &mut symbols)?;
            }
            "lua" => {
                self.extract_lua_symbols(root_node, source, &mut symbols)?;
            }
            "dockerfile" => {
                self.extract_dockerfile_symbols(root_node, source, &mut symbols)?;
            }
            // "markdown" => {
    ...
    // === New language symbol extractors ===
    ...
    fn extract_scala_symbols(
            ...
            "class_definition" | "object_definition" | "trait_definition" => {
                if let Some(name_node) = node.child_by_field_name("name") {
                    ...
                    symbols.push(Symbol {
                        ...
                        range,
                        selection_range: self.node_to_range(&name_node, source)?,
                        detail: Some(node.kind().replace("_definition", "").to_string()),
                    });
                }
            }
            _ => {}
        ...
        for child in node.children(&mut node.walk()) {
            self.extract_scala_symbols(child, source, symbols)?;
        }
    ...
    fn extract_kotlin_symbols(
            ...
            "class_declaration" | "object_declaration" | "interface_declaration" => {
                if let Some(name_node) = node.child_by_field_name("simple_identifier") {
                    ...
                    symbols.push(Symbol {
                        ...
                        range,
                        selection_range: self.node_to_range(&name_node, source)?,
                        detail: Some(node.kind().replace("_declaration", "").to_string()),
                    });
                }
            }
            _ => {}
        ...
        for child in node.children(&mut node.walk()) {
            self.extract_kotlin_symbols(child, source, symbols)?;
        }
    ...
    fn extract_csharp_symbols(
        ...
        match node.kind() {
            ...
            "class_declaration" | "struct_declaration" | "interface_declaration" => {
                if let Some(name_node) = node.child_by_field_name("name") {
                    ...
                    symbols.push(Symbol {
                        name: name.to_string(),
                        kind: if node.kind() == "struct_declaration" {
                            SymbolKind::STRUCT
                        } else {
                            SymbolKind::CLASS
                        },
        ...
        for child in node.children(&mut node.walk()) {
            self.extract_csharp_symbols(child, source, symbols)?;
        }
    ...
    fn extract_swift_symbols(
        ...
        for child in node.children(&mut node.walk()) {
            self.extract_swift_symbols(child, source, symbols)?;
        }
    ...
    fn extract_elixir_symbols(
        ...
        match node.kind() {
            "call" => {
                // Look for def, defp, defmodule, defmacro
                if let Some(target) = node.child_by_field_name("target") {
                    if let Ok(func_name) = target.utf8_text(source.as_bytes()) {
                        if matches!(func_name, "def" | "defp" | "defmodule" | "defmacro") {
                            // Get the first argument which is the name
                            for child in node.children(&mut node.walk()) {
                                if child.kind() == "arguments" {
                                    if let Some(first_arg) = child.named_child(0) {
                                        if let Ok(name) = first_arg.utf8_text(source.as_bytes()) {
                                            let range = self.node_to_range(&node, source)?;
                                            symbols.push(Symbol {
                                                name: name.to_string(),
                                                kind: if func_name == "defmodule" {
                                                    SymbolKind::MODULE
                                                } else {
                                                    SymbolKind::FUNCTION
                                                },
                                                range,
                                                selection_range: self.node_to_range(&first_arg, source)?,
                                                detail: Some(func_name.to_string()),
                                            });
                                        }
                                    }
                                    break;
        ...
        for child in node.children(&mut node.walk()) {
            self.extract_elixir_symbols(child, source, symbols)?;
        }

        Ok(())
    }

    fn extract_ocaml_symbols(
        &self,
        node: tree_sitter::Node,
        source: &str,
        symbols: &mut Vec<Symbol>
        ...
        match node.kind() {
            ...
            "type_definition" | "module_definition" => {
                if let Some(name_node) = node.child_by_field_name("name") {
                    let name = name_node.utf8_text(source.as_bytes())?;
                    let range = self.node_to_range(&node, source)?;
                    symbols.push(Symbol {
                        name: name.to_string(),
                        kind: if node.kind() == "module_definition" {
                            SymbolKind::MODULE
                        } else {
                            SymbolKind::CLASS
                        },
        ...
        for child in node.children(&mut node.walk()) {
            self.extract_ocaml_symbols(child, source, symbols)?;
        }

        Ok(())
    }

    fn extract_erlang_symbols(
        &self,
        node: tree_sitter::Node,
        source: &str,
        symbols: &mut Vec<Symbol>
        ...
        match node.kind() {
            "function_clause" | "function_declaration" => {
                if let Some(name_node) = node.child_by_field_name("name") {
                    let name = name_node.utf8_text(source.as_bytes())?;
                    let range = self.node_to_range(&node, source)?;
                    symbols.push(Symbol {
            ...
            "module_attribute" => {
                // Extract module name
                if let Some(name_node) = node.child_by_field_name("name") {
                    if let Ok(attr_name) = name_node.utf8_text(source.as_bytes()) {
                        if attr_name == "module" {
                            if let Some(value_node) = node.child_by_field_name("value") {
                                if let Ok(module_name) = value_node.utf8_text(source.as_bytes()) {
                                    let range = self.node_to_range(&node, source)?;
                                    symbols.push(Symbol {
                                        name: module_name.to_string(),
                                        kind: SymbolKind::MODULE,
                                        range,
                                        selection_range: self.node_to_range(&value_node, source)?,
        ...
        for child in node.children(&mut node.walk()) {
            self.extract_erlang_symbols(child, source, symbols)?;
        }

        Ok(())
    }

    fn extract_julia_symbols(
        &self,
        node: tree_sitter::Node,
        source: &str,
        symbols: &mut Vec<Symbol>
            ...
            "struct_definition" | "abstract_definition" | "module_definition" => {
                if let Some(name_node) = node.child_by_field_name("name") {
                    ...
                    symbols.push(Symbol {
                        ...
                        kind: if node.kind() == "module_definition" {
                            SymbolKind::MODULE
                        } else if node.kind() == "struct_definition" {
                            SymbolKind::STRUCT
                        } else {
                            SymbolKind::CLASS
                        },
        ...
        for child in node.children(&mut node.walk()) {
            self.extract_julia_symbols(child, source, symbols)?;
        }
    ...
    fn extract_lua_symbols(
        ...
        match node.kind() {
            ...
            "variable_declaration" => {
                // Extract local/global variable declarations
                if let Some(name_node) = node.child_by_field_name("name") {
                    let name = name_node.utf8_text(source.as_bytes())?;
                    let range = self.node_to_range(&node, source)?;
                    symbols.push(Symbol {
        ...
        for child in node.children(&mut node.walk()) {
            self.extract_lua_symbols(child, source, symbols)?;
        }
    ...
    fn extract_dockerfile_symbols(
        ...
        match node.kind() {
            "from_instruction" | "label_instruction" | "env_instruction" | "arg_instruction" => {
                // Extract Dockerfile instructions as symbols
                if let Ok(instruction) = node.utf8_text(source.as_bytes()) {
                    let range = self.node_to_range(&node, source)?;
                    ...
                    symbols.push(Symbol {
                        ...
                        range,
                        selection_range: range,
                        detail: Some(node.kind().replace("_instruction", "").to_uppercase()),
                    });
                }
            }
            _ => {}
        ...
        for child in node.children(&mut node.walk()) {
            self.extract_dockerfile_symbols(child, source, symbols)?;
        }
